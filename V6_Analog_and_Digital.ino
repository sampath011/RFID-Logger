...
// Time delays
unsigned long previousMillis_Analog = 0;
unsigned long interval_Analog = 1000;  
...
int analogStatus[sizeof(analogPins)];
int digitalStatus[sizeof(digitalPins)] = {0};
...
void analogCheck(){
  ...
  for (int i = 0; i < sizeof(analogPins); i++) {
    analogStatus[i] = analogRead(analogPins[i]);
  }
  analogJSON = "{\"Type\": \"Analog\", \"Time\": " + String(timeString) + ", \"Pin 32\": " + String(analogStatus[0]) + ", \"Pin 33\": " + String(analogStatus[1]) + ", \"Pin 34\": " + String(analogStatus[2]) + ", \"Pin 35\": " + String(analogStatus[3]) + "}";
  client.publish(topic, analogJSON.c_str());
}

void digitalCheck(int pin){

  // Publish digital pin status
  digitalJSON = "{\"Type\": \"Digital\", \"Change Detected on Pin\": " + String(pin) + ", \"Time\": " + String(timeString) + "}";
  client.publish(topic, digitalJSON.c_str());
}
...
void loop() {
  ...
  if (currentMillis - previousMillis_Analog >= interval_Analog) {
    analogCheck();
    previousMillis_Analog = currentMillis;
  }
  ...
  for (int i = 0; i < sizeof(digitalPins); i++) {
    ...
    // Detect Rising Edges
    if (currentState == HIGH && digitalStatus[i] == LOW) {
      digitalCheck(digitalPins[i]);
    }
    // Update Digital Pin State
    digitalStatus[i] = currentState;
  }
